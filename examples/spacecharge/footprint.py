    ...
    return np.array(
        [
            [
                ...
                for r in np.linspace(r_min, r_max, r_N)
            ]
            for theta in np.linspace(theta_min, theta_max, theta_N)
        ]
    )
...
def initial_xy_cartesian(x_min, x_max, x_N, y_min, y_max, y_N):
    return np.array(
        [
            [(x, y) for x in np.linspace(x_min, x_max, x_N)]
...
def draw_footprint(A, axis_object=None, figure_object=None, axis=0, linewidth=4):
    ...
    p_collection = matplotlib.collections.PatchCollection(
        patches, facecolors=[], linewidth=linewidth, edgecolor=patch_colors
    )
...
def example():
    coords_matrix = initial_xy_polar(0.01, 5, 11, 0.0, np.pi / 2.0, 10)
